...
@property (strong, nonatomic) UITextView *sickHistoryText;
@property (weak, nonatomic) UIButton *endEditBtn;
...
- (void)keyboardFramHasChanged:(NSNotification *)notice {
    ...
    if (rect.origin.y > 600) {
        UIButton *endEditBtn = [[UIButton alloc]initWithFrame:[UIScreen mainScreen].bounds];
        self.endEditBtn = endEditBtn;
        endEditBtn.backgroundColor = [UIColor clearColor];
        [self.view addSubview:endEditBtn];
        [endEditBtn addTarget:self action:@selector(endEditBtnClick) forControlEvents:UIControlEventTouchDown];
        [self.view bringSubviewToFront:endEditBtn];
        [self.view bringSubviewToFront:self.sickHistoryText];
    }else {
        self.endEditBtn.alpha = 0;
...
- (void)decorateUI {
    ...
    self.sickHistoryText = sickHistoryText;
